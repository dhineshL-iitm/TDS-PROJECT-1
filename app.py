...
async def get_embedding(text, max_retries=3):
    ...
    while retries < max_retries:
        try:
            ...
            url = "http://aipipe.org/openai/v1/embeddings"
            ...
            error_msg = (
                f"Exception getting embedding (attempt {retries+1}/{max_retries}): {e}"
            )
            logger.error(error_msg)
            logger.error(traceback.format_exc())
            retries += 1
...
async def find_similar_content(query_embedding, conn):
    try:
        ...
        for chunk in discourse_chunks:
            try:
                ...
                if processed_count % 1000 == 0:
                    logger.info(
                        f"Processed {processed_count}/{len(discourse_chunks)} discourse chunks"
                    )
            ...
            try:
                ...
                if similarity >= SIMILARITY_THRESHOLD:
                    ...
                    if not url or not url.startswith("http"):
                        ...
                        url = (
                            f"https://docs.onlinedegree.iitm.ac.in/{chunk['doc_title']}"
                        )
                ...
                if processed_count % 1000 == 0:
                    logger.info(
                        f"Processed {processed_count}/{len(markdown_chunks)} markdown chunks"
                    )
        ...
        logger.info(
            f"Returning {len(final_results[:MAX_RESULTS])} final results after grouping"
        )
...
async def generate_answer(question, relevant_results, max_retries=2):
    ...
    while retries < max_retries:
        try:
            ...
            url = "http://aipipe.org/openai/v1/chat/completions"
            headers = {"Authorization": API_KEY, "Content-Type": "application/json"}
            payload = {
                "model": "gpt-4o-mini",
                "messages": [
...
async def process_multimodal_query(question, image_base64):
    ...
    try:
        logger.info(
            f"Processing query: '{question[:50]}...', image provided: {image_base64 is not None}"
        )
        if not image_base64:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"Received image description: '{image_description[:50]}...'"
                    )
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error processing image (status {response.status}): {error_text}"
                    )
...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        logger.info(
            f"Received query request: question='{request.question[:50]}...', image_provided={request.image is not None}"
        )
        ...
        try:
            ...
            logger.info(
                f"Returning result: answer_length={len(result['answer'])}, num_links={len(result['links'])}"
            )
